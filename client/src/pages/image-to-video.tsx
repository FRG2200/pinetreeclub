...
export default function ImageToVideoPage() {
  ...
  const [aspectRatio, setAspectRatio] = useState(presetExtra.aspect_ratio || "9:16");
  ...
  const videoGenerations = generations.filter(g => g.type === "image2video");
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-96 border-r border-border p-6 space-y-6 overflow-y-auto">
        <h1 className="text-xl font-bold">画像から動画生成</h1>

        {/* Image Upload - Start and End Frames */}
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <span className="text-sm font-medium">最初のフレーム</span>
            <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileUpload} className="hidden" />
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1 text-xs" type="button" onClick={() => fileInputRef.current?.click()}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          >
            {generateMutation.isPending ? "生成中..." : "作成 ⬦ 6"}
          </Button>
          <p className="text-xs text-center text-muted-foreground">想定生成時間: 15s 〜 42s</p>
        </form>
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        {videoGenerations.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-96 text-muted-foreground">
            ...
            <p>まだ生成された動画がありません</p>
            <p className="text-sm">画像をアップロードして生成を開始してください</p>
          </div>
        ) : (
          <div className="space-y-4">
              ...
              <Card key={gen.id} className="bg-card border-border" data-testid={`generation-card-${idx}`}>
                <CardContent className="p-4">
                  <div className="flex items-start gap-4 mb-3">
                    <Badge variant="secondary">画像から動画生成</Badge>
