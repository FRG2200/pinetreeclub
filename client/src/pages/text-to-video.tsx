...
import { Sparkles, ChevronDown, Heart, Edit, RefreshCw, MoreHorizontal, Play } from "lucide-react";
...
export default function TextToVideoPage() {
  ...
  useEffect(() => {
    if (videoModels.length > 0 && !selectedModel) {
      if (presetModel) {
        const match = videoModels.find(m => m.name.toLowerCase().includes(presetModel.toLowerCase()));
        if (match) { setSelectedModel(match.name); return; }
      }
      setSelectedModel(videoModels[0].name);
    }
  }, [videoModels]);
  const [aspectRatio, setAspectRatio] = useState(presetExtra.aspect_ratio || "16:9");
  const [resolution, setResolution] = useState(presetExtra.resolution || "720P");
  const [duration, setDuration] = useState(presetExtra.duration || "5s");
  ...
  const videoGenerations = generations.filter(g => g.type === "text2video");
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-96 border-r border-border p-6 space-y-6 overflow-y-auto">
        <h1 className="text-xl font-bold">テキストから動画生成</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            <Textarea
              {...register("prompt")}
              placeholder="閑静しているシーンを書いてみてください"
              ...
              <Card key={gen.id} className="bg-card border-border" data-testid={`generation-card-${idx}`}>
                <CardContent className="p-4">
                  <div className="flex items-start gap-4 mb-3">
                    <Badge variant="secondary">テキストから動画生成</Badge>
                  ...
                  {gen.status === "completed" ? (
                    <div className="relative rounded-lg overflow-hidden bg-muted aspect-video flex items-center justify-center">
                      {gen.videoUrl ? (
                        ...
                        <img src={gen.imageUrl} alt={gen.prompt} className="w-full h-full object-contain" />
                      ) : (
